...
//
// IMPORTANT: nothing in this module touches `process.env`, instantiates
// Google auth clients, or does any network/credential work at import time.
// Every value below is computed lazily, inside a function, the first time
// it's actually needed — so this module can be safely imported during
// `next build` (or any other static analysis pass) on a machine that has
// no environment variables configured at all, and only requires them at
// request time once deployed.

const GAM_SCOPE = "https://www.googleapis.com/auth/dfp";

function getGamConfig() {
  const apiVersion = process.env.GAM_API_VERSION || "v202408";
  return {
    apiVersion,
    namespace: `https://www.google.com/apis/ads/publisher/${apiVersion}`,
    soapEndpoint: `https://ads.google.com/apis/ads/publisher/${apiVersion}`,
    applicationName: process.env.GAM_APPLICATION_NAME || "AdStreamHQ",
  };
}
...
/**
 * Lazily builds (and caches) the Google JWT auth client. `google-auth-library`
 * is dynamically imported here — not at the top of the module — so nothing
 * about Google auth is touched until a request actually calls this.
 */
...
function buildSoapEnvelope({ networkCode, bodyXml, namespace, applicationName }) {
...
<soap-env:Envelope xmlns:soap-env="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ns1="${namespace}">
  <soap-env:Header>
    <ns1:RequestHeader soap-env:actor="http://schemas.xmlsoap.org/soap/actor/next" soap-env:mustUnderstand="0">
      ...
      <ns1:applicationName>${escapeXml(applicationName)}</ns1:applicationName>
    </ns1:RequestHeader>
  </soap-env:Header>
  <soap-env:Body>
    ${bodyXml}
...
async function soapRequest({ service, soapAction, accessToken, networkCode, bodyXml }) {
  ...
  const { namespace, soapEndpoint, applicationName } = getGamConfig();
  const envelope = buildSoapEnvelope({ networkCode, bodyXml, namespace, applicationName });

  let response;
  try {
    response = await fetch(`${soapEndpoint}/${service}`, {
      method: "POST",
      headers: {
...
export async function runReportJob({ accessToken, networkCode, reportQueryXml }) {
  const { namespace } = getGamConfig();
  const bodyXml = `<runReportJob xmlns="${namespace}">
      <reportJob>
...
export async function getReportJobStatus({ accessToken, networkCode, reportJobId }) {
  const { namespace } = getGamConfig();
  const bodyXml = `<getReportJobStatus xmlns="${namespace}">
...
export async function getReportDownloadUrl({
  ...
  const { namespace } = getGamConfig();
  const bodyXml = `<getReportDownloadURL xmlns="${namespace}">
      <reportJobId>${escapeXml(reportJobId)}</reportJobId>
